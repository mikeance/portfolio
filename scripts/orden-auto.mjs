// Escribe catalogo/orden-auto.json: el orden automático actual de FACES y LIFE (el mismo que usa la web)
...
const { photos, sortLife, sortFaces } = await import(join(ROOT, 'src/lib/photos.ts'));
...
console.log(`✔ orden automático: Faces ${out.P.length} · Life ${out.L.length}`);
